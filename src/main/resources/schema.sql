...
(
  id          uuid            not null  comment 'pk' primary key,
  concert_id  uuid           not null comment '콘서트 fk',
  comment     varchar(1023)   not null comment '댓글 내용',
  ...
  updated_at  datetime(6) null,
  updated_by  varchar(255) null,
  constraint fk_concert_comments_concert_id foreign key (concert_id) references concerts (id)
) comment '콘서트 테이블';

create table if not exists reservation
(
  id          uuid   not null  comment 'pk' primary key,
  seat_id     uuid   not null comment '좌석 pk',
  user_id     uuid   not null comment '예약자',
